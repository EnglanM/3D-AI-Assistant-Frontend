...
function Avatar({ avatar_url, speak, setSpeak,chats , setChats, text, setAudioSource, playing }) {
  ...
  useEffect(() => {
    ...
    if (speak === false)
     ...
     makeTest(text).then(response => {
      const botResponse = response.data.choices[0].message.content;
      setChats(chats => [...chats, { msg: botResponse, who: 'bot' }])
      makeSpeech(response.data.choices[0].message.content)
      .then(response => {
...
function makeTest(message) {
  return axios.post(host+'/testChatbot', { message })
}

function App() {

  const [chats, setChats] = useState([{ msg: 'Say something!', who: 'bot', exct: '0' }]);
    ...
    <div className="full">
      ...
      <div className='chat-div'>
        <div className='chat-box'>
          {chats.map((chat) => {
            if (chat.who === "me") {
              ...
              return <p className={chat.who}>
                {chat.msg}
                <div className='time'>{"generated by the " + chat.who}</div>
              </p>
            }
          })}
        ...
        <div className='msg-box'>
          ...
          <input type='text' ref={inputRef} value={msg} onChange={e => setMsg(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') { getResposnse(inputRef.current.value) } }} placeholder='Say Hello!'></input>
          <button className='msgbtn' id='send' onClick={() => { getResposnse(inputRef.current.value) }}>
